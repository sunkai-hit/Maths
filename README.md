...
没有第三方运行时依赖，不需要 `npm install`。练习卷可直接下载为 A4 PDF，无需打开打印窗口。也可点击「打印」选择打印机；应用内浏览器未弹出窗口时，请下载 PDF 后用 PDF 阅读器打印。

## 已实现

- 六章教材目录、26 节和六章复习；按内容拆分建议课时。
- 64 个题型、64 道讲解例题和 192 道练习题；第一章已按完整学习材料扩充到 22 个题型、22 道例题和 66 道分层练习。
...
- 独立生成 PDF、实际分页预览与页码；中文、数学符号及几何图形均离线生成。
...
| `work/*.mjs` | 创建过程中使用的预览、检查、离线导出和历史打包脚本 |
...
应用当前从 `dist/data.js` 加载课程与题库构造器，第一章题库拆分在 `chapter1a.js`～`chapter1d.js`，第二至第六章位于 `chapter2.js`～`chapter6.js`，`reviews.js` 保存单元综合复习，`diagrams.js` 补充可结构化表达的示意图。修改题库后运行 `npm run export:data` 与 `npm run build:offline`，同步导出 JSON、SQL 与单文件离线应用。

当前没有后端数据库、用户表或服务器端学生记录。`database/question-bank.sql` 是结构化数据导出；应用不会自动连接此数据库。
...
第一章题库依据用户提供的《鲁教七上数学（1三角形）》完整材料整理，保留知识点、巩固训练、尺规作图、全等测距与本章专题的教学逻辑；少量依赖原图或存在 OCR 符号错位的题干，仅做数字化表述和符号统一。第二至第六章仍为首批原创内容。难度分级以应用内题型递进为主，不代表原材料给出的官方难度标定。

已完成数据结构检查、部分数值核对和主要操作的浏览器检查；没有进行全题库独立教研审核。教材章、节顺序来自已有项目课程目录，第一章专题内容归入该章“单元复习”入口以兼容当前导航结构。
